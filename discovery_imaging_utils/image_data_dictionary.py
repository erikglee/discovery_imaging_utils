	...
	if 'nifti_data_path' in file_path_dictionary.keys():
		...
		if nifti_data.ndim > 2:
			...
			if type(nifti_inclusion_inds) != type(None):
				...
				nifti_data = np.reshape(nifti_data[nifti_inclusion_inds], (nifti_inclusion_inds[0].shape[0], nifti_data.shape[-1]))
...
def _populate_general_info_dict(confounds_dict, file_path_dict, TR):
    ...
    if 'lh_func_path' in file_path_dict.keys():

        temp_path = file_path_dict['lh_func_path'].split('/')[-1]
        split_end_path = temp_path.split('_')

    else:

        temp_path = file_path_dict['nifti_func_path'].split('/')[-1]
        split_end_path = temp_path.split('_')
