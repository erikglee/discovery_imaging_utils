...
def calc_alignment_metrics(reference_data, aparcaseg_data, brainmask_data, deviation_power = 1):


    gm_mask = np.zeros(reference_data.shape)

    #Find gm/wm inds and calculate metric
    gm_inds = np.where(np.logical_or(np.logical_and(aparcaseg_data>=1000, aparcaseg_data<=1035),np.logical_and(aparcaseg_data>=2000, aparcaseg_data<=2035)))
    wm_inds = np.where(np.logical_or(aparcaseg_data == 2, aparcaseg_data == 41))

    gm_var = mean_pow_deviation(reference_data[gm_inds], deviation_power)
    wm_var = mean_pow_deviation(reference_data[wm_inds], deviation_power)
    total_gmwmvar = mean_pow_deviation(np.hstack((reference_data[gm_inds],reference_data[wm_inds])), deviation_power)
    gmwm_var_component_ratio = (gm_var + wm_var)/(2*total_gmwmvar)
    ...
    brain_var = mean_pow_deviation(reference_data[brain_inds], deviation_power)
    other_var = mean_pow_deviation(reference_data[other_inds], deviation_power)
    total_var = mean_pow_deviation(np.hstack((reference_data[brain_inds],reference_data[other_inds])), deviation_power)
    brainmask_var_component_ratio = (brain_var + other_var)/(2*total_var)
    ...
    gm_mask[gm_inds] = 1
    gm_skin_1_dilation = binary_dilation(gm_mask, iterations=1)
    gm_skin_2_dilation = binary_dilation(gm_mask, iterations=2)
    gm_skin_1_dilation[np.where(aparcaseg_data > 0)] = False #remove any brain tissues
    gm_skin_2_dilation[np.where(aparcaseg_data > 0)] = False #remove any brain tissues
    gm_skin_1_dilation_inds = np.where(gm_skin_1_dilation > 0)
    gm_skin_2_dilation_inds = np.where(gm_skin_2_dilation > 0)

    gm_skin_1_dilation_var = mean_pow_deviation(reference_data[gm_skin_1_dilation_inds], deviation_power)
    gm_skin_2_dilation_var = mean_pow_deviation(reference_data[gm_skin_2_dilation_inds], deviation_power)
    gm_skin_1_dilation_and_gm_var = mean_pow_deviation(np.hstack((reference_data[gm_skin_1_dilation_inds],reference_data[gm_inds])), deviation_power)
    gm_skin_2_dilation_and_gm_var = mean_pow_deviation(np.hstack((reference_data[gm_skin_2_dilation_inds],reference_data[gm_inds])), deviation_power)
    gm_skin_1dil_var_component_ratio = (gm_var + gm_skin_1_dilation_var)/(2*gm_skin_1_dilation_and_gm_var)
    gm_skin_2dil_var_component_ratio = (gm_var + gm_skin_2_dilation_var)/(2*gm_skin_2_dilation_and_gm_var)

    return gmwm_var_component_ratio, brainmask_var_component_ratio, gm_skin_1dil_var_component_ratio, gm_skin_2dil_var_component_ratio
